...
public abstract class AbstractGridRobot implements EmbodiedAgent {
  ...
  public record RobotSnapshot(
      Voxel.VoxelSnapshot[][][] voxelSnapshots, List<Action>[][][] lastStepActions)
      implements AgentSnapshot {
    public RobotSnapshot(Voxel.VoxelSnapshot[][][] voxelSnapshots) {
      this(
          voxelSnapshots,
          new List[voxelSnapshots.length][voxelSnapshots[0].length][voxelSnapshots[0][0].length]);
    ...
    public List<ObjectSnapshot> components() {
      return Arrays.stream(voxelSnapshots)
          .flatMap(aa -> Arrays.stream(aa).flatMap(Arrays::stream))
          .filter(Objects::nonNull)
          .collect(Collectors.toList());
    }
  ...
  public AgentSnapshot snapshot(Ode4jEngine engine) {
    Voxel.VoxelSnapshot[][][] snapshotGrid =
        new Voxel.VoxelSnapshot[grid.length][grid[0].length][grid[0][0].length];
