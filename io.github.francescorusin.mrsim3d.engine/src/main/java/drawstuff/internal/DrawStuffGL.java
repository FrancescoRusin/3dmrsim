...
public class DrawStuffGL extends LwJGL implements DrawStuffApi {
  ...
  private static class ImageIO implements Image {
    ...
    ImageIO(String filename) {
      ...
      try {
        ...
        // check values
        if (image_width < 1 || image_height < 1) dsError("bad image file \"%s\"", filename);
        if (max_value != 255) dsError("image file \"%s\" must have color range of 255", filename);
        ...
        if (f.read(buf, 0, image_width * image_height * 3 * 1) != image_width * image_height * 3)
  ...
  private static class Texture {
    ...
    public Texture(String filename) {
      ...
      // GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0​, 0, 0, image.width(), image.height(),
      // GL11.GL_BGRA, GL11.GL_UNSIGNED_BYTE, image.data());
      ...
      GL11.glTexImage2D(
          ...
          image.data()); // ByteBuffer.wrap(image.data().array()));
  ...
  private void drawConvex(
      float[] _planes, int _planecount, float[] _points, int _pointcount, int[] _polygons) {
    // unsigned
    int polyindex = 0;
  ...
  //			unsigned int _pointcount,
  //			unsigned int *_polygons)
  private void drawConvexD(
      double[] _planes, int _planecount, double[] _points, int _pointcount, int[] _polygons) {
    // unsigned
    int polyindex = 0;
    for (int i = 0; i < _planecount; ++i) {
      // unsigned
  ...
  private void drawTriangle(
      final float[] vAll, final int v0, final int v1, final int v2, boolean solid) {
  ...
  private void drawCylinder(float l, float r, float zoffset) {
    ...
    for (i = 0; i <= n; i++) {
      ...
      GL11.glNormal3d(0, 0, 1);
      GL11.glVertex3d(ny * r, nz * r, l + zoffset);
      if (i == 1 || i == n / 2 + 1) setColor(color[0], color[1], color[2], color[3]);
  ...
  private void drawGround() {
    ...
    GL11.glBegin(GL11.GL_QUADS);
    GL11.glNormal3f(0, 0, 1);
    GL11.glTexCoord2f(-gsize * ground_scale + ground_ofsx, -gsize * ground_scale + ground_ofsy);
  ...
  private void setShadowDrawingMode() {
    ...
    if (use_textures) {
      ...
      GL11.glDisable(GL11.GL_TEXTURE_2D);
      GL11.glColor3f(
          GROUND_R * SHADOW_INTENSITY, GROUND_G * SHADOW_INTENSITY, GROUND_B * SHADOW_INTENSITY);
    }
  ...
  public void dsSimulationLoop(String[] args, int window_width, int window_height, dsFunctions fn) {
    ...
    if (fn.dsGetVersion() > DS_VERSION) dsDebug("bad version number in dsFunctions structure");
  ...
  @Override
  public void dsDrawTriangles(
      final float[] pos, final float[] R, final float[][] v, boolean solid) {
    if (current_state != 2) dsError("drawing function called outside simulation loop");
    setupDrawingMode();
    GL11.glShadeModel(GL11.GL_FLAT);
    setTransform(pos, R);
  ...
  public void dsDrawTriangles(
      final DVector3C pos, final DMatrix3C R, final DVector3C[] v, boolean solid) {
    int i;
    ...
    GL11.glShadeModel(GL11.GL_FLAT);
    setTransform(pos2, R2);
    for (i = 0; i < v.length; ++i) drawTriangle(v[3 * i], v[3 * i + 1], v[3 * i + 2], solid);
    GL11.glPopMatrix();
  }
   ...
   */
  @Override
  public void dsDrawCylinder(final DVector3C pos, final DMatrix3C R, float length, float radius) {
    float[] pos2 = pos.toFloatArray4();
    float[] R2 = R.toFloatArray12();
   ...
   */
  @Override
  public void dsDrawCapsule(final DVector3C pos, final DMatrix3C R, float length, float radius) {
    float[] pos2 = pos.toFloatArray4();
    float[] R2 = R.toFloatArray12();
