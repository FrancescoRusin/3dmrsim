...
public class VisualTest extends DrawStuff.dsFunctions {
  ...
  public void step(boolean pause) {
    ...
    for (DFixedJoint joint :
        engine.fixedJoints.values().stream()
            .filter(Objects::nonNull)
            .flatMap(List::stream)
            .toList()) {
      dsDrawLine(joint.getBody(0).getPosition(), joint.getBody(1).getPosition());
    }
    List<? extends DJoint> agentsInternalJoints =
        engine.agents.stream()
            .map(EmbodiedAgent::components)
            .flatMap(Collection::stream)
            .map(v -> ((Voxel) v).internalJoints())
            .flatMap(List::stream)
            .toList();
    for (DDoubleBallJoint joint :
        engine.springJoints.values().stream()
            .filter(Objects::nonNull)
            .flatMap(List::stream)
            .toList()) {
